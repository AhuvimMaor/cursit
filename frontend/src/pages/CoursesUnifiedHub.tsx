...
export const CoursesUnifiedHub = ({ user }: CoursesUnifiedHubProps) => {
  ...
  const usedPhaseTypes = useMemo(() => {
    ...
    for (const row of hubRows) {
      for (const p of row.phases) s.add(p.phaseType);
    }
    return [...s].sort();
  }, [hubRows]);
      ...
      return (
        ...
        >
          ...
          <div className='grid grid-cols-1 gap-2.5 overflow-visible sm:grid-cols-2 sm:gap-3 lg:grid-cols-4'>
            {rows.map((row, rowIndexInMonth) => {
              const colInRowLg = rowIndexInMonth % 4;
              const isCardOpen = expandedId === row.instanceId;
              ...
              return (
                <div
                  key={row.instanceId}
                  className={`relative flex min-w-0 flex-col gap-2.5 ${isCardOpen ? 'lg:z-20' : ''}`}
                >
                  <div
                    className={`flex min-h-0 flex-col rounded-xl border transition-all ${
                      isDone
                        ? 'border-dashed border-slate-400 bg-gradient-to-br from-slate-200/85 to-slate-100/60 shadow-inner'
                        : 'border-border bg-white shadow-sm'
                    } ${
                      isCardOpen && !isDone
                        ? 'border-primary/40 ring-2 ring-primary/20'
                        : isCardOpen && isDone
                          ? 'border-slate-500 ring-2 ring-slate-300/40'
                          : !isDone
                  ...
                  >
                    <div
                      ...
                      onKeyDown={(e) => {
                        if (e.key === 'Enter' || e.key === ' ') {
                          e.preventDefault();
                          setExpandedId(isCardOpen ? null : row.instanceId);
                        }
                      }}
                      onClick={(e) => {
                      ...
                      className={`flex flex-1 cursor-pointer flex-col p-3 text-right ${
                    ...
                    >
                      <div className='mb-2 flex items-start justify-between gap-1.5'>
                        ...
                        <ChevronDown
                          size={17}
                          className={`mt-0.5 shrink-0 transition-transform ${isCardOpen ? 'rotate-180' : ''} ${isDone ? 'text-slate-500' : 'text-muted-foreground'}`}
                          aria-hidden
                        />
                      </div>
                  ...
                  {isCardOpen && (
                    <div
                      className={`rounded-2xl border-2 p-4 shadow-inner sm:p-5 lg:z-10 lg:min-w-0 lg:w-[calc(200%+0.75rem)] ${
                        colInRowLg === 3 ? 'lg:relative lg:start-[calc(-100%-0.75rem)]' : ''
                      } ${
                        hubArchivedRow(row)
                          ? 'border-dashed border-slate-400 bg-slate-300/35'
                          : 'border-primary/25 bg-gradient-to-b from-slate-50/90 to-white'
                      }`}
                      id={`course-detail-${row.instanceId}`}
                    >
                      <div
                        className={`mb-3 flex items-start gap-2 rounded-lg border px-2.5 py-2 text-right ${
                          hubArchivedRow(row)
                            ? 'border-slate-400/70 bg-slate-200/50'
                            : 'border-primary/30 bg-primary/[0.09]'
                        }`}
                        aria-live='polite'
                      >
                        <CornerDownLeft
                          className={`mt-0.5 h-4 w-4 shrink-0 ${hubArchivedRow(row) ? 'text-slate-600' : 'text-primary'}`}
                          aria-hidden
                        />
                        <div className='min-w-0 flex-1'>
                          <p
                            className={`text-[10px] font-bold uppercase tracking-wide ${hubArchivedRow(row) ? 'text-slate-600' : 'text-primary'}`}
                          >
                            פירוט למחזור שנבחר
                          </p>
                      ...
                      <div className='mb-4 flex flex-wrap items-start justify-between gap-3 border-b border-border/80 pb-3'>
                        <div>
                          <p className='text-xs font-medium text-muted-foreground'>מחזור נבחר</p>
                          <p className='text-lg font-bold text-foreground'>{row.courseName}</p>
                          <p className='text-sm font-semibold text-primary'>{row.instanceName}</p>
                        </div>
                        <div className='flex shrink-0 flex-wrap items-center justify-end gap-2'>
                          {showRegister(row) && rowRegStatus == null && (
                            <button
                              type='button'
                              onClick={() => handleRegister(row.instanceId)}
                              disabled={registering === row.instanceId}
                          ...
                          <button
                            ...
                            className='rounded-lg border border-border bg-white px-3 py-1.5 text-xs font-medium text-muted-foreground hover:bg-muted'
                          >
                            סגור
                          </button>
                        </div>
                      ...
                      <div className='mt-3 flex flex-wrap gap-3 text-xs text-muted-foreground'>
                        {row.gmushHours != null && row.gmushHours > 0 && (
                          <span className='flex items-center gap-1'>
                            <Clock size={12} />
                            {row.gmushHours} שעות גמו&quot;ש
                          </span>
                        )}
                        {row.requirements && (
                          <span className='flex items-center gap-1'>
                      ...
                      {isAdmin && (
                        <div className='mt-4 rounded-xl border border-border bg-muted/10 p-1'>
