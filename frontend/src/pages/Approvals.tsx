...
export const Approvals = ({ user }: ApprovalsProps) => {
  ...
  const modalTitle = action?.type === 'reject' ? 'דחיית בקשה' : action?.type === 'prioritize' ? 'תיעדוף ושליחה' : 'אישור';
  ...
  return (
    <div className='space-y-5'>
            ...
            <div key={r.id} className='flex flex-col rounded-xl border border-border bg-white p-4 shadow-sm transition-all hover:shadow-md hover:border-primary/20'>
              ...
              <div className='mt-4 flex flex-wrap gap-2 border-t border-border pt-3'>
                {user.role === Role.BRANCH_COORD ? (
                  <>
                    <button type='button' onClick={() => setAction({ id: r.id, type: 'prioritize' })} className='rounded-lg bg-blue-500 px-3 py-1.5 text-xs font-medium text-white hover:bg-blue-600'>
                      תעדוף ושלח
                    </button>
      ...
      <Modal open={action !== null} onClose={() => { setAction(null); setNotes(''); setPriority(''); }} title={modalTitle} size='sm'>
        <div className='space-y-3'>
          {action?.type === 'prioritize' && (
            <div>
              <label className='mb-1 block text-xs font-medium text-foreground'>תיעדוף (1 = גבוה)</label>
              <input type='number' min={1} value={priority} onChange={(e) => setPriority(e.target.value)} className='w-full rounded-lg border border-border bg-white px-3 py-2 text-sm outline-none focus:border-primary' autoFocus />
            </div>
          )}
          <div>
            ...
            <textarea value={notes} onChange={(e) => setNotes(e.target.value)} rows={2} placeholder={action?.type === 'reject' ? 'חובה...' : ''} className='w-full rounded-lg border border-border bg-white px-3 py-2 text-sm outline-none focus:border-primary' autoFocus={action?.type !== 'prioritize'} />
          </div>
          <div className='flex justify-end gap-2'>
            <button type='button' onClick={() => { setAction(null); setNotes(''); setPriority(''); }} className='rounded-lg border border-border px-3 py-1.5 text-sm text-muted-foreground hover:bg-muted'>ביטול</button>
            <button
              type='button'
              onClick={handleSubmit}
              disabled={submitting || (action?.type === 'reject' && !notes) || (action?.type === 'prioritize' && !priority)}
